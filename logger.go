...
type Logger struct {
	...
	// or navigate to: https://gitee.com/menciis/logx/issues/3#issuecomment-355895870.
	NewLine bool
...
func (l *Logger) SetNoColor() *Logger {
	l.mu.Lock()
	l.TimeColor = false
	l.MessageColor = false
	l.PrefixColor = false
	l.LevelColor = false
	l.mu.Unlock()
...
var logHijacker = func(ctx *pio.Ctx) {
	...
	if t := l.FormatTime(); t != "" {
		if level, ok := Levels[l.Level]; ok && logger.TimeColor {
			if l.Level != SilentLevel {
				pio.WriteRich(w, t, level.ColorCode, level.Style...)
			}
		} else {
			fmt.Fprint(w, t)
		}
	...
	if prefix := logger.Prefix; len(prefix) > 0 {
		if level, ok := Levels[l.Level]; ok && logger.PrefixColor {
			pio.WriteRich(w, prefix, level.ColorCode, level.Style...)
		} else {
			fmt.Fprint(w, prefix)
		}
	}
	if logger.MessageColor {
		if l.Level != DisableLevel {
			if level, ok := Levels[l.Level]; ok {
				pio.WriteRich(w, l.Message, level.ColorCode, level.Style...)
				w.Write(spaceBytes)
			}
		}
	} else {
...
func (l *Logger) Println(v ...interface{}) {
	l.print(SilentLevel, fmt.Sprint(v...), true, nil)
}
...
func (l *Logger) Silent(v ...interface{}) {
	l.Log(SilentLevel, v...)
}

// Silentf exactly the same as `Logf`.
func (l *Logger) Silentf(format string, args ...interface{}) {
	l.Logf(SilentLevel, format, args...)
}

// Verbose exactly the same as `Log`.
func (l *Logger) Verbose(v ...interface{}) {
	l.Log(VerboseLevel, v...)
}

// Verbosef exactly the same as `Logf`.
